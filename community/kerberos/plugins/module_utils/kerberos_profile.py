...
def generate_profile(data):
    return """
# Config generated by ansible community.kerberos collection.
# Any edits to these files might be overwritten.
#
    """ + "\n".join([
...
def remove_krb5_realm_config(
    ...
    for section_name, section_data in existing_config.items():
        if section_name in global_section_names:
            new_config[section_name] = section_data
            continue

        for config_realm_name, realm_data in section_data.items():
            if config_realm_name != realm_name:
                new_config[config_realm_name] = realm_data
